...
class ProfileMainFriendActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        deleteFriend(myUserId, FriendName)
    }
        ...
        binding.friendRemoveButton.setOnClickListener {
            ...
            query.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    val userInfoRef = FirebaseDatabase.getInstance().reference.child("UserInfo")
                    val myNameRef = userInfoRef.child(myName.convertStrToBase64())
    ...
    private fun addFriend(myName: String, friendName: String) {
        binding.friendAddButton.setOnClickListener {
            database.child("UserInfo").child(myName.convertStrToBase64()).child("userFriendsInfo").push().setValue(friendName)
                .addOnSuccessListener {
                    // Increase friendCount in UserInfo by 1
                    database.child("UserInfo").child(myName.convertStrToBase64()).child("friendCount")
                        .addListenerForSingleValueEvent(object : ValueEventListener {
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
